...
<style>
  ...
  .badge{background:#1e3a5f;color:#7eb8f7;padding:.15rem .5rem;border-radius:3px;display:inline-block;}
...
fetch('/builder').then(r=>r.json()).then(d=>{
  ...
  for(const [k,v] of Object.entries(r)){
    prefs.innerHTML+=`<div class="field"><span class="field-label">${k}</span><span class="field-value badge">${v}</span></div>`;
  }
});
</script>
</body>
