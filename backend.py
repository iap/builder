...
def chat(
    ...
    if r.status_code != 200:
        ...
        if any(k in err_low for k in ("subscri", "accessdenied", "not entitled", "not activated", "free tier", "q developer")):
            raise RuntimeError(
...
def _extract_answer_with_conversation_id(response: requests.Response) -> tuple[str, Optional[str], Optional[str]]:
    ...
    for chunk in response.iter_content(chunk_size=4096):
        ...
        total += len(chunk)
        if total > 2_000_000:
            raise RuntimeError("Q response stream exceeded max size")
        chunks.append(chunk)
