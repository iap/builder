...
Legacy migration: the provider slug was renamed `aws-build` → `aws-builder` for naming consistency. `_provider.py` detects and migrates entries written under the old slug so existing config isn't orphaned, and `unregister_provider()` removes entries at both slugs. Additionally, entries written by old `setup.sh` (with `key_env: AWS_BUILD_ADAPTER_DUMMY` or `127.0.0.1` host) are adopted and rewritten on `register_provider()`.
