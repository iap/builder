...
# Always back up first, matching setup.sh and the README's backup contract.
BACKUP="${CONFIG}.bak.$(date +%Y%m%d_%H%M%S)"
cp "$CONFIG" "$BACKUP"
echo "✓ backed up config → $BACKUP"

# Remove the providers: builder block (the 'builder:' key under 'providers:',
# plus its child lines). Match on stripped line == 'builder:' (unique key,
# any indentation).
python3 - "$CONFIG" <<'PY'
import sys
cfg = sys.argv[1]
...
for ln in lines:
    ...
    if drop:
        if ln and not ln.startswith("  "):
            drop = False       # next top-level/sibling key -> stop dropping
        else:
