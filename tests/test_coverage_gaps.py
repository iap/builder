...
def test_provider_register_stamps_adapter_port(monkeypatch, tmp_path):
    """register_provider must persist the adapter port so a later uninstall
    recognises the entry even when AWS_BUILD_ADAPTER_PORT is no longer set
    (Greptile P1)."""
    from _provider import _is_our_base_url, register_provider
    ...
    # The stamped port is now ours; an unbacked loopback port stays foreign.
    assert _is_our_base_url("http://localhost:9999/v1")
    assert not _is_our_base_url("http://localhost:7777/v1")
...
def test_handle_bid_login_already_authenticated():
    ...
    with patch.object(
        plugin,
        "start_login",
        return_value={"already_authenticated": True, "phase": "authenticated"},
    ):
...
def test_provider_unregister_removes_our_entry(monkeypatch):
    ...
    cfg = {
        "providers": {_provider.PROVIDER_SLUG: {"base_url": "http://localhost:8088/v1"}}
    }
    fake_hermes, fake_cfg, saved = _make_hermes_cli_mock(cfg)
    monkeypatch.setitem(sys.modules, "hermes_cli", fake_hermes)
    monkeypatch.setitem(sys.modules, "hermes_cli.config", fake_cfg)
...
def test_provider_unregister_leaves_foreign_entry(monkeypatch):
    ...
    cfg = {
        "providers": {_provider.PROVIDER_SLUG: {"base_url": "http://example.com/v1"}}
    }
    fake_hermes, fake_cfg, _saved = _make_hermes_cli_mock(cfg)
    monkeypatch.setitem(sys.modules, "hermes_cli", fake_hermes)
    monkeypatch.setitem(sys.modules, "hermes_cli.config", fake_cfg)
