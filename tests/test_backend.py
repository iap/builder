...
def test_chat_raises_on_missing_access_token(monkeypatch):
    """`access_token` must be present before signing the request."""
...
def test_chat_surfaces_403_accessdenied_with_not_entitled(monkeypatch):
    class _SubResp:
        status_code = 403

        @property
        def text(self):
            return '{"__type":"AccessDeniedException","message":"You are not entitled to use Q Developer"}'
